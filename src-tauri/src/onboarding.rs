...
pub fn onboarding_set_skills_dir(
  ...
  onboarding_set_skills_dir_with_home(&default_home_dir(), &dir, create_if_missing.unwrap_or(false))
}
...
mod tests {
  ...
  fn onboarding_set_skills_dir_returns_skill_list_and_persists() {
    ...
    let result = onboarding_set_skills_dir_with_home(&home, &skills_root.to_string_lossy(), false)
  ...
  fn onboarding_set_skills_dir_creates_missing_directory_when_allowed() {
    ...
    let result = onboarding_set_skills_dir_with_home(&home, &skills_root.to_string_lossy(), true)
      .expect("set skills dir");
    assert!(result.success);
    assert!(skills_root.exists());
