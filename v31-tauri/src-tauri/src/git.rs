...
pub fn get_git_status(root: &Path) -> Result<GitStatusResult, String> {
  let repo = git2::Repository::open(root)
    .map_err(|e| format!("Open git repository failed: {e}"))?;
...
pub fn commit_all(_root: &Path, _message: &str) -> Result<GitCommitResult, String> {
  ...
  let repo = git2::Repository::open(_root)
    .map_err(|e| format!("Open git repository failed: {e}"))?;
  ...
  let mut index = repo.index().map_err(|e| format!("Open git index failed: {e}"))?;
  index
    .add_all(["*"].iter(), git2::IndexAddOption::DEFAULT, None)
    .map_err(|e| format!("Stage files failed: {e}"))?;
  index.write().map_err(|e| format!("Write git index failed: {e}"))?;
  ...
  let signature = repo.signature().or_else(|_| {
    git2::Signature::now("MySkills Manager", "noreply@myskills-manager.local")
  })
